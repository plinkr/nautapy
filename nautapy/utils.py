...
def strtime2seconds(str_time):
    ...
    if not res:
        raise NautaFormatException(
            "El formato del intervalo de tiempo es incorrecto: {}".format(str_time)
        )

    return int(res["hours"]) * 3600 + int(res["minutes"]) * 60 + int(res["seconds"])
